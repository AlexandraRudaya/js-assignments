...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
    var num = Math.pow((x2-x1),2) + Math.pow((y2-y1),2);
    num = Math.sqrt(num);
    return num;
    throw new Error('Not implemented');
}
...
function getParallelipidedDiagonal(a,b,c) {
    var a = Math.pow(a,2)+Math.pow(b,2)+Math.pow(c,2);
    a = Math.sqrt(a)
    return a;
    //or return Math.hypot(a,b,c);
    throw new Error('Not implemented');
}
